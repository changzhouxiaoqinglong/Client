...
namespace NWH.VehiclePhysics
{
    ...
    /// </summary>
    public class CameraChanger : MainCameraItemBase
    {
        /// <summary>
        ...
        {
            EventDispatcher.GetInstance().AddEventListener(EventNameList.CHANGE_VIEW, GetChangeViewEv);
        }
        ...
        void Update()
        {
            if (vehicleController.Active)
            {
                bool changeCamera = Keyboard.current.cKey.wasPressedThisFrame;//Input.GetKeyDown(KeyCode.C);
                //try
                //{
        ...
        /// 切换视角
        /// </summary>
        private void GetChangeViewEv(IEventParam param)
        {
            NextCamera();
        }
        ...
        {
            return vehicleCameras[currentCamera].GetComponent<Camera>();
        }

        protected override void SetCameraStormPlane()
        {
            base.SetCameraStormPlane();
            foreach (var item in vehicleCameras)
            {
                item.GetComponent<Camera>().farClipPlane = UniStorm.UniStormSystem.CAMERA_FAR_PLANES;
        ...
        {
            EventDispatcher.GetInstance().RemoveEventListener(EventNameList.CHANGE_VIEW, GetChangeViewEv);
        }
    }
}
