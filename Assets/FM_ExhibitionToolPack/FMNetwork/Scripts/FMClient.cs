...
public class FMClient : MonoBehaviour
{
    ...
    IEnumerator NetworkClientStart()
    {
        ...
        else
        {
            ...
            while (FmLoom.numThreads >= FmLoom.maxThreads) yield return null;
            FmLoom.RunAsync(() =>
            {
                //client request
                while (!stop)
        ...
        FmLoom.RunAsync(() =>
        {
            while (!stop)
            {
                //这里不要加打印日志，会导致画面无法同步，原因不明！
                //Logger.LogDebug("FMCLient: Update receive logic");
                try
                {
                    ...
                    if (!FoundServer)
                    {
                        ...
                        FoundServer = true;
                        Logger.LogWarning(TAG + "FoundServer!!!");
                    }
                ...
                {
                    Logger.LogWarning(TAG + "Client Receiver Timeout: " + socketException.ToString());
    ...
    void Sender()
    {
            ...
            else
            {
                ...
                if (_appendQueueSendPacket.Count > 0)
                {
                    ...
                    while (_appendQueueSendPacket.Count > 0 && k < 30)
                    {
                        ...
                        Logger.LogDebug(TAG + "Send TO ServerIp : " + ServerIP);
                        LastSentTimeMS = Environment.TickCount;
                    }
                }
                else
        ...
        {
            Logger.LogWarning(TAG + "client sender timeout: " + socketException.ToString());
